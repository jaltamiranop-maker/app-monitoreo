...
def guardar_pedido(cliente, producto, datos_dict, resultado_list):
    ...
    cursor.execute('''
        INSERT INTO pedidos 
        (fecha_registro, cliente, producto, datos_completos, resultado_optimizacion)
        VALUES (?, ?, ?, ?, ?)
    ''', (
...
def obtener_detalle_pedido(id_pedido):
    ...
    cursor.execute(
        'SELECT datos_completos, resultado_optimizacion FROM pedidos WHERE id = ?', 
        (id_pedido,)
    )
